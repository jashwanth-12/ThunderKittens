...
__global__ void kernel(__nv_bfloat16* A, __nv_bfloat16* B, __nv_bfloat16* C, int N) {
    ...
    int aCol = blockIdx.z * blockDim.y + threadIdx.y;
    int bRow = blockIdx.z * blockDim.x + threadIdx.x;
    ...
    float sum = 0.0;
    for (int i = 0; i < 16; i++) {
        sum += __bfloat162float(ATile[threadIdx.x][i] * BTile[i][threadIdx.y]);
    }
